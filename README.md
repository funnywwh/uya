...
Uya 是一个系统编程语言，专注于**内存安全**、**并发安全**和**零运行时开销**。设计目标是提供 Rust 级别的安全性，同时保持 C 级别的性能和简洁性。
...
> **Uya 0.12 = 默认即 Rust 级内存安全 + 并发安全 + Zig 风格错误处理 + Python 风格切片 + 安全指针算术**；
...
**注意**：Uya 语言目前处于开发阶段（**0.12 版本**），部分特性可能尚未完全实现。请参考 [uya.md](./uya.md) 了解当前版本的限制和未来计划。
